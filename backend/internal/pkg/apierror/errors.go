...
var (
	...
	ErrValidationFailed   = New(http.StatusUnprocessableEntity, "VALIDATION_FAILED", "Request validation failed")
	...
	ErrPasswordRequired   = New(http.StatusBadRequest, "PASSWORD_REQUIRED", "Password login not available for OAuth accounts")
	...
	// File/storage errors
	ErrFileTooLarge      = New(http.StatusRequestEntityTooLarge, "FILE_TOO_LARGE", "File exceeds the maximum allowed size")
	ErrUnsupportedFormat = New(http.StatusUnsupportedMediaType, "UNSUPPORTED_FORMAT", "File format is not supported")
	ErrStorageQuota      = New(http.StatusForbidden, "STORAGE_QUOTA_EXCEEDED", "Storage quota exceeded")

	// Backup errors
	ErrBackupNotFound   = New(http.StatusNotFound, "BACKUP_NOT_FOUND", "Backup not found")
	ErrBackupInProgress = New(http.StatusConflict, "BACKUP_IN_PROGRESS", "A backup operation is already in progress")
	ErrRestoreFailed    = New(http.StatusInternalServerError, "RESTORE_FAILED", "Database restore failed")
)
